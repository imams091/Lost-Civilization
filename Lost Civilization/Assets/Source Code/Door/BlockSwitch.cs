...
{
    public BlockTriggerDoor[] doorTriggers;
    ...
    void OnTriggerEnter2D(Collider2D target)
    {
        ...
        foreach (BlockTriggerDoor trigger in doorTriggers)
    ...
    void OnDrawGizmos()
    { //menggambar garis penanda doortrigger yg bersangkutan
        Gizmos.color = sticky ? Color.blue : Color.green; //kalo biru artinya switch tsb sticky, kalo hijau tidak sticky
        foreach (BlockTriggerDoor trigger in doorTriggers)
        { //cari semua doortrigger pada array
            if (trigger != null)
                Gizmos.DrawLine(transform.position, trigger.doors.transform.position); //gambar garisnya dari posisi si switch
        }
    }
}
